...
func ThemeQuery(c *scheduler.Context) {
	...
	if util.HasPrefixIn(arg, "新增", "添加") {
		...
		keyword := params[1]
		value := params[2]
		err := dao.CreateTheme(keyword, value)
		if err != nil {
			_, _ = c.Reply(err.Error())
		} else {
			msg := fmt.Sprintf("添加主题「%s」成功!\n", keyword)
	...
	if util.HasPrefixIn(arg, "更新", "修改") {
		...
		keyword := params[1]
		value := params[2]
		err := dao.UpdateTheme(keyword, value)
		if err != nil {
			_, _ = c.Reply(err.Error())
			return
		} else {
			msg := fmt.Sprintf("更新主题「%s」成功!\n", keyword)
	...
	if util.HasPrefixIn(arg, "删除", "移除") {
		...
		keyword := params[1]
		err := dao.DeleteThemeByKeyword(keyword)
		if err != nil {
			_, _ = c.Reply(err.Error())
			return
		}

		_, _ = c.Reply(fmt.Sprintf("删除主题「%s」成功!", keyword))
		return
	}

	keywords, err := dao.LoadThemeKeywords()
	if err != nil {
		logger.Error("获取主题关键词列表失败 %v", err)
	...
	if len(matchList) == 0 {
		...
		result, err := dao.GetThemeByKeyword(matchList[0])
		if err != nil {
			_, _ = c.Reply(e.SystemErrorNote)
			return
		}
...
func TimeLimitingTaskQuery1(c *scheduler.Context) {
	logger.Info("限时任务攻略查询")

	themies := new(database.Theme)
	has, err := dao.DB.Where("keyword = ?", "限时任务").Get(themies)
	if err != nil {
	...
	if !has {
		...
		logger.Debug("找到一个攻略:", themies.Value)
		_, _ = c.Reply(themies.Value)
	}
}
